...
const Q = `query($limit:Int!,$offset:Int!){ pokemon_v2_item(order_by:{id:asc}, limit:$limit, offset:$offset){ id name cost pokemon_v2_itemcategory{name} pokemon_v2_itemnames(where:{language_id:{_in:[6,9]}}){name language_id} } }`
...
const out = all.map((it) => ({ id: it.id, n: it.name, de: nm(it.pokemon_v2_itemnames, 6) || nm(it.pokemon_v2_itemnames, 9), en: nm(it.pokemon_v2_itemnames, 9) || nm(it.pokemon_v2_itemnames, 6), c: it.pokemon_v2_itemcategory?.name || 'other', cost: it.cost ?? 0 }))

...
import { ChevronDown, User as UserIcon, Settings, LogOut, ScrollText, MessageCircle, Users, Download } from 'lucide-react'
        ...
        {open && (
          <div role="menu" className="absolute right-0 top-full mt-2 z-[1000] bg-[#1c1c26] border border-[#2e2e42] rounded-2xl shadow-2xl overflow-hidden min-w-56 anim-pop" onMouseEnter={cancelClose} onMouseLeave={scheduleClose}>
            ...
            <div className="py-1.5">
              ...
              {IN_COMPANION_WINDOW
                ? <button role="menuitem" onClick={() => { setOpen(false); navigate('/mysetup') }} className={item}><Users className="w-4 h-4" /> Mein Setup</button>
                : <button role="menuitem" onClick={() => openLink(LINKS.download)} className={item}><Download className="w-4 h-4" /> Companion herunterladen</button>}

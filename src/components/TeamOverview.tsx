...
function GhostSlot({ mon, game, currentLocationName, onImport }: { mon: EmulatorMon; game: string; currentLocationName: string | null; onImport: (prefill: EncounterPrefill, route?: string) => void }) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      // Sichere Route-Quelle: Fangort (metLocation) zuerst, sonst aktueller Ort.
  ...
  return (
    <div className="rounded-xl border border-dashed border-yellow-700/40 bg-[#1c1c20] p-3">
      <div className="h-1 rounded-full mb-2 bg-slate-700/40" />
      <div className="flex items-center gap-2">
        <img src={getSpriteUrl(mon.speciesId)} alt="" className="w-12 h-12 object-contain shrink-0 opacity-70" />
        <div className="min-w-0 flex-1">
          <div className="text-slate-300 text-sm font-bold capitalize truncate">
            {speciesName ?? `#${String(mon.speciesId).padStart(3, '0')}`}
          </div>
          <div className="flex items-center gap-2 text-[10px]">
            <span className="text-slate-400">Lv {mon.level}</span>
            <span className="px-1 rounded font-bold text-yellow-500 bg-yellow-500/10">Noch nicht übernommen</span>
          </div>
        </div>
      </div>
      {hpPct != null && (
      ...
      <button
        ...
        disabled={importing}
        className="mt-2 w-full text-[10px] font-bold px-2 py-1.5 rounded-lg transition-all hover:opacity-80 disabled:opacity-40"
...
export default function TeamOverview({ myEncounters, partnerEncounters, teamSlots, players, myPlayerId, game, onSelectEncounter, onImport }: Props) {
  const { team: liveTeam, connected, currentLocationName } = useEmuTeamStore()
  ...
  const displaySlots: DisplaySlot[] = (() => {
    ...
    // Connected: build slots by live team slot number (1-6).
    ...
    for (let slotNum = 1; slotNum <= 6; slotNum++) {
      ...
      if (!liveMon) {
        result.push({ type: 'empty', idx: slotNum - 1 })
        continue
      }
      const enc = liveMon.pid != null
        ? teamEncs.find((e) => e.emu_pid === String(liveMon.pid))
        : undefined
      if (enc) {
        usedEncIds.add(enc.id)
        result.push({ type: 'enc', enc, mon: liveMon })
      } else {
        result.push({ type: 'ghost', mon: liveMon })
      }
    }

    // Append any teamEncs that are only in team_slots (no live slot) — keep them visible.
  ...
  // Count only imported encounters in the team for the header badge.
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {displaySlots.map((slot) => {
            ...
            if (slot.type === 'ghost') {
              return <GhostSlot key={`ghost-${slot.mon.slot}`} mon={slot.mon} game={game} currentLocationName={currentLocationName} onImport={onImport} />
            }
            return (

...
// SoulGuide workspace tab (no overlay, no second analysis screen). The later live-SoulGuide
// updates only this component.
interface Props {
...
export default function SoulGuidePanel({ data, onSelectEncounter }: Props) {
  ...
  if (empty) {
    return (
      <div className="rounded-3xl border border-white/10 p-12 text-center" style={{ background: 'rgba(20,20,28,0.6)' }}>
  ...
  return (
    <div className="space-y-4 anim-fade-up">
      ...
      <div className="rounded-3xl border border-white/10 p-5 flex items-center gap-4" style={{ background: 'linear-gradient(180deg, rgba(20,20,28,0.9), rgba(12,12,18,0.9))' }}>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        {/* Nächster Arenaleiter */}
        {gyms.length > 0 && (
          <Card icon={<Trophy className="w-4 h-4" />} title="Nächster Arenaleiter · Vorbereitung" accent="#FFCB05" full>
        ...
        {/* Team-Balance + Rollen */}
        <Card icon={<Activity className="w-4 h-4" />} title="Team-Balance · Rollen" accent="#60a5fa">
          {b.withStats === 0 ? <Empty /> : (
            <>
        ...
        <Card icon={<ShieldCheck className="w-4 h-4" />} title="Typanalyse · Defensive" accent="#f87171">
        ...
        {/* Utility */}
        <Card icon={<Sparkles className="w-4 h-4" />} title="Utility" accent="#c084fc" full={!hasPartner}>
        ...
        {/* SoulLink-Analyse */}
        {hasPartner && (
          <Card icon={<Link2 className="w-4 h-4" />} title="SoulLink-Analyse · Synergie" accent="#a78bfa">
      ...
      {/* Typeffektivität — aufklappbarer Abschnitt (nimmt keinen Dauerplatz weg) */}
